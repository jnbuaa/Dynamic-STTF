...
#        loss = self.critic.train_on_batch([X1, X2], y)
    ...
    def play(self, episode):
        ...
        print('Final SEKNN: RMSE=%.4f,MAE=%.4f,MAPE=%.4f'%(RMSE,MAE,MAPE))
        RMSE_, MAE_, MAPE_ = self.env.get_error(np.array(self.env.set_predY_).reshape(-1, 257),np.array(self.env.set_realY).reshape(-1, 257))
        print('Final ISEKNN: RMSE=%.4f,MAE=%.4f,MAPE=%.4f'%(RMSE_,MAE_,MAPE_))
        with open('model_/log.txt','a') as f:
            f.write('\n*** Episode=%d ***'%episode)
            f.write('\n SEKNN: RMSE=%.4f,MAE =%.4f,MAPE =%.4f'%(RMSE,MAE,MAPE))
            f.write('\nISEKNN: RMSE=%.4f,MAE_=%.4f,MAPE_=%.4f'%(RMSE_,MAE_,MAPE_))
        ...
        df.to_csv('model_/%d-seknn-predY%dp%d.csv'%(episode, self.env.inputstep, self.env.predstep), header=False, index=False)
